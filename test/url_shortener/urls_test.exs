defmodule UrlShortener.UrlsTest do
  ...
  describe "urls" do
    ...
    test "create_url/1 with valid data creates a url" do
      valid_attrs = %{url: "some url", short_code: "some short_code", access_count: 42}

      assert {:ok, %Url{} = url} = Urls.create_url(valid_attrs)
      assert url.url == "some url"
      assert url.short_code == "some short_code"
      assert url.access_count == 42
    end
    ...
    test "update_url/2 with valid data updates the url" do
      url = url_fixture()
      update_attrs = %{url: "some updated url", short_code: "some updated short_code", access_count: 43}

      assert {:ok, %Url{} = url} = Urls.update_url(url, update_attrs)
      assert url.url == "some updated url"
      assert url.short_code == "some updated short_code"
      assert url.access_count == 43
    end
